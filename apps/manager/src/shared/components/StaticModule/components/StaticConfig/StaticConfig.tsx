...
export function StaticConfig() {
    const { itemId } = useParams({ from: "/modules/_provider/$moduleId/_formProvider/edit/static/$itemId/" });
    ...
    if (error) {
        return (
            <div className="p-4">
                <ErrorPage error={error.message || i18n.t("Error loading item")} />
            </div>
        );
    }
    ...
    return (
        <FormProvider {...form}>
            <div>
                <Button onClick={() => navigate({to:"/modules/$moduleId/edit"})} icon={<IconArrowLeft16 />}>
                    {i18n.t("Back")}
                </Button>
                <PageHeader title={item?.title ?? ""} actions={<ItemActions />} />
                <StaticForm />
            </div>
        </FormProvider>
    );
