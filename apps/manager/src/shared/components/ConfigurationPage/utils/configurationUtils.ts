...
export const useConfiguration = () => {
    ...
    const getKeysInNamespace = async (
        namespace: string,
        ...
        try {
            ...
            return Array.isArray(result)
                ? result.filter((k): k is string => typeof k === 'string')
                : []
        } catch (error) {
            if (error.details?.httpStatusCode === 404) {
                addLog(
                    `Namespace '${namespace}' not found or is empty.`,
                    'info-low'
                )
                return []
            }
            addLog(
                `Error fetching keys for ${namespace}: ${error.message}`,
                'error'
            )
            throw error
        }
    ...
    const getValue = async <T>(
        ...
        try {
            ...
            addLog(
                `Error fetching value for ${namespace}/${key}: ${error.message}`,
                'error'
            )
            throw error
        }
    ...
    const setValue = async (
        ...
        try {
            ...
            if (addLog) {
                addLog(
                    `Successfully set value for ${namespace}/${key}`,
                    'success'
                )
            }
        } catch (error) {
            if (addLog) {
                addLog(
                    `Error setting value for ${namespace}/${key}: ${error.message}`,
                    'error'
                )
            }
            throw error
    ...
    const deleteKey = async (
        ...
        try {
            ...
            if (error.details?.httpStatusCode === 404) {
                addLog(
                    `Key '${key}' not found in namespace '${namespace}'. No action taken.`,
                    'info-low'
                )
                return
            }
            addLog(
                `Error deleting key ${namespace}/${key}: ${error.message}`,
                'error'
            )
            throw error
        }
    ...
    const fetchDocumentDetails = async (
        ...
        try {
            ...
            if (
                ...
                addLog(
                    `No valid details found for document ID: ${documentId}`,
                    'warning'
                )
                return null
            }
            ...
            if (!id || !name) {
                addLog(
                    `Incomplete document details for ID: ${documentId}`,
                    'warning'
                )
                return null
            }
            ...
            addLog(
                `Error fetching details for document ID: ${documentId}: ${error.message}`,
                'error'
            )
            return null
        }
    ...
    const fetchDocumentData = async (
        ...
        try {
            ...
            if (!(result instanceof Blob)) {
                addLog(
                    `Invalid data format for document ID: ${documentId} - expected Blob`,
                    'warning'
                )
                return null
            }
            ...
            addLog(
                `Error fetching data for document ID: ${documentId}: ${error.message}`,
                'error'
            )
            return null
        }
    ...
    const deleteDocument = async (
        ...
        try {
            ...
            if (error.details?.httpStatusCode === 404) {
                if (addLog) {
                    addLog(
                        `Document '${document.name}' not found. No action taken.`,
                        'info-low'
                    )
                }
                return true
            ...
            if (addLog) {
                addLog(
                    `Error deleting document ${document.id} (${document.name}): ${error.message}`,
                    'error'
                )
            }
            return false
    ...
    const uploadDocument = async (
        ...
        try {
            ...
            addLog(
                `Successfully uploaded document ${document.name}`,
                'info-low'
            )
            return true
        } catch (error) {
            addLog(
                `Error uploading document ${document.id} (${document.name}): ${error.message}`,
                'error'
            )
            return false
        }
