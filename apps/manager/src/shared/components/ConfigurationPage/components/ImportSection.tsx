...
export const ImportSection = ({ setLogs }: ImportSectionProps) => {
    const { setValue, addLog, uploadDocument, deleteDocument } =
        useConfiguration()
                ...
                if (
                    ...
                    for (const doc of documents) {
                        if (doc.attachment && doc.name) {
                            ...
                            if (assetFile) {
                                addLog(setLogs)(
                                    `Uploading file resource for ${doc.name}...`,
                                    'info-low'
                                )
                                ...
                                if (fileResourceId) {
                                    ...
                                    addLog(setLogs)(
                                        `Failed to upload file resource for ${doc.name}`,
                                        'warning'
                                    )
                                    continue
                                }
                                ...
                                addLog(setLogs)(
                                    `Asset file ${assetFilePath} not found in ZIP`,
                                    'warning'
                                )
                                continue
                            }
                            ...
                            if (!success) {
                                addLog(setLogs)(
                                    `Skipping document ${doc.name} due to upload failure`,
                                    'warning'
                                )
                            }
                        }
                    ...
                    addLog(setLogs)(
                        'Invalid or empty documents.json',
                        'warning'
                    )
                }
            } else {
