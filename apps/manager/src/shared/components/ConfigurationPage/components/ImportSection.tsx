import { Button, FileInputField, NoticeBox, IconArrowUp16 } from '@dhis2/ui';
...
export const ImportSection = ({ setLogs }: ImportSectionProps) => {
    ...
    const handleFileChange = ({ files }: { files: FileList | null }) => {
        if (files && files.length > 0 && files[0].type === 'application/zip') {
            ...
            addLog(setLogs)('Invalid file type. Only ZIP files are allowed.', 'error');
    ...
    const handleImport = useCallback(async () => {
        if (!selectedFile) {
            addLog(setLogs)('No file selected for import.', 'warning');
            return;
        }
        setLoading(true);
        addLog(setLogs)(`Import process started for ${selectedFile.name}...`, 'info');
        const zip = new JSZip();
                    ...
                    if (namespace === DatastoreNamespaces.MAIN_CONFIG && typeof data === 'object' && !Array.isArray(data)) {
                        for (const key in data) {
                            if (Object.values(DatastoreKeys).includes(key as DatastoreKeys)) {
                                await setValue(namespace, key, data[key], addLog(setLogs));
                                addLog(setLogs)(`Imported key '${key}' into ${namespace}.`, 'info-low');
                            }
                        }
                    } else if (Array.isArray(data)) {
                        for (const item of data) {
                            if (item?.id) {
                                await setValue(namespace, item.id, item, addLog(setLogs));
                                addLog(setLogs)(`Imported key '${item.id}' into ${namespace}.`, 'info-low');
                            }
                        }
                    }
                }
