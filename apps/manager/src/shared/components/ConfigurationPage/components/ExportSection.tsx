    ...
    const handleExport = useCallback(async () => {
        ...
        try {
                    ...
                    for (const key of Object.values(DatastoreKeys)) {
                        ...
                        if (value) {
                            ...
                            if (key === DatastoreKeys.APPEARANCE) {
                                ...
                                if (
                                    appearance.header?.style?.trailingLogo?.url
                                ) {
                                    extractedIds.documents.push(
                                        appearance.header.style.trailingLogo.url
                                    )
                                }
                            }
                    ...
                    for (const key of keys) {
                        ...
                        if (item) {
                            ...
                            if (namespace === DatastoreNamespaces.MODULES) {
                                ...
                                if (
                                    module.type === ModuleType.DOCUMENTS &&
                                    module.config
                                ) {
                                    const config =
                                        module.config as DocumentsModuleConfig
                                ...
                                if (
                                    ...
                                    (module.config as StaticModuleConfig)
                                        ?.namespace &&
            ...
            for (const id of uniqueIds) {
                ...
                const docData = await fetchDocumentData(
                    id,
                    details!.name,
                    addLog(setLogs)
                )
            ...
            zip.file(
                'documents.json',
                JSON.stringify({ documents: documentDetails }, null, 2)
            )
