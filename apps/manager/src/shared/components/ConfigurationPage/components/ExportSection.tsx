      ...
      for (const namespace of Object.values(DatastoreNamespaces)) {
        ...
        if (namespace === DatastoreNamespaces.MAIN_CONFIG) {
          ...
          for (const key of Object.values(DatastoreKeys)) {
            const value = await getValue(namespace, key, addLog(setLogs));
            if (value) data[key] = value;
          }
          zip.file(`${namespace}.json`, JSON.stringify(data, null, 2));
        } else {
          const keys = await getKeysInNamespace(namespace, addLog(setLogs));
          const items: any[] = [];
          for (const key of keys) {
            const item = await getValue(namespace, key, addLog(setLogs));
            if (item) {
              items.push(item);
              if (namespace === DatastoreNamespaces.MODULES && (item as AppModule).type === ModuleType.STATIC && ((item as AppModule).config as StaticModuleConfig)?.namespace) {
                const staticNamespace = ((item as StaticModule).config as StaticModuleConfig).namespace;
                if (!exportedStaticNamespaces.has(staticNamespace)) {
                  const staticKeys = await getKeysInNamespace(staticNamespace, addLog(setLogs));
                  const staticItems = await Promise.all(staticKeys.map((sKey) => getValue(staticNamespace, sKey, addLog(setLogs))));
                  zip.file(`${staticNamespace}.json`, JSON.stringify(staticItems.filter(Boolean), null, 2));
                  exportedStaticNamespaces.add(staticNamespace);
                }
              }
            }
          }
          zip.file(`${namespace}.json`, JSON.stringify(items, null, 2));
        }
      }
      const blob = await zip.generateAsync({ type: 'blob' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `datastore_export_${new Date().toISOString().split('T')[0]}.zip`;
      link.click();
