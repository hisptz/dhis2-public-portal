...
export interface SortManualProps {
	onClose(): void;
	items: DocumentItem[];
	onSubmit(updatedItems: DocumentItem[]): void;
	hide: boolean;
}

const reorder = (
	list: DraggableDocumentItem[],
	startIndex: number,
	endIndex: number,
...
type DraggableDocumentItem = DocumentItem & { draggableId: string };

export function SortDocumentForm({
	items: initialItems,
	onClose,
	onSubmit,
	hide,
	...
	const [currentItems, setCurrentItems] = useState<DraggableDocumentItem[]>(
		[],
	);
	...
	const handleSave = async () => {
		...
		onSubmit(itemsToSubmit as DocumentItem[]);
	...
	const getDraggableId = (item: DraggableDocumentItem): string => {
	...
	return (
		<Modal position="middle" hide={hide} onClose={handleCloseClick}>
			<ModalTitle>{i18n.t("Sort Document Items")}</ModalTitle>
			<ModalContent>
				<DragDropContext onDragEnd={handleOnDragEnd}>
					<Droppable droppableId="DocumentItemsSortableList">
						{(provided) => (
							<div
