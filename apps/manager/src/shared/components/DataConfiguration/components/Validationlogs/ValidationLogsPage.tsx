...
import { useValidationLogs, useValidationStatus, useValidationDiscrepancies, useRerunValidation, useAnalyticsLastRun } from './hooks/validation';
...
export function ValidationLogsPage({ configId }: ValidationLogsPageProps) {
    ...
    const validationDiscrepancies = useValidationDiscrepancies(configId, { limit: 1000 });
    const rerunValidation = useRerunValidation(configId);
    const analyticsLastRun = useAnalyticsLastRun();

    const handleRerun = async () => {
        if (!validationStatus.data) return;

        try {
            const lastValidationParams = localStorage.getItem(`validation-params-${configId}`);
            const params = lastValidationParams ? JSON.parse(lastValidationParams) : {
                dataItemsConfigIds: [],
                runtimeConfig: {
                    pageSize: 10,
    ...
    const isValidationRunning = validationStatus.data?.status === DataServiceRunStatus.RUNNING ||
        validationStatus.data?.status === DataServiceRunStatus.QUEUED;

    const hasDiscrepancies = (validationDiscrepancies.data?.summary?.total ?? 0) > 0;
    ...
    if (validationStatus.isError) {
        return (
            <NoticeBox error title={i18n.t('Failed to load validation status')}>
                {validationStatus.error?.message || i18n.t('An error occurred while loading the validation status')}
            </NoticeBox>
        );
    }
        ...
        <div className="space-y-4">
            ...
            <Card className="p-4">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h3 className="text-lg font-medium">{i18n.t('Data Validation')}</h3>
                        {analyticsLastRun.data?.lastAnalyticsTableSuccess && (
                            <p className="text-sm text-gray-600 mt-1">
                                {i18n.t('Destination analytics last run:')} {new Date(analyticsLastRun.data.lastAnalyticsTableSuccess).toLocaleString()}
                            </p>
                        )}
                    </div>
                    <Button
