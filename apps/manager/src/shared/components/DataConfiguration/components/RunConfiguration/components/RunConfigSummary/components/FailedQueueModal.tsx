...
function getProcessTypeFromQueue(queueName: string): string {
    ...
    if (normalizedQueue.includes('metadata-download'))
...
function getQueuePatternFromProcessType(processType: string): string {
    ...
    if (
        normalizedType.includes('metadata') &&
        normalizedType.includes('download')
    )
        return 'metadata-download'
    if (
        normalizedType.includes('metadata') &&
    ...
    if (normalizedType.includes('data') && normalizedType.includes('download'))
        return 'data-download'
    if (normalizedType.includes('data') && normalizedType.includes('upload'))
        return 'data-upload'
    if (normalizedType.includes('data') && normalizedType.includes('delete'))
...
function filterMessagesByProcessType(
    messages: any[],
    processType?: string
): any[] {
    ...
    return messages.filter((message) => {
        ...
        return (
            queueLower.includes(queuePattern) &&
...
export function FailedQueueModal({
    ...
    const handleRetryByProcessType = async () => {
        if (processType) {
            const processTypeMapping: Record<
                string,
                | 'data-upload'
                | 'metadata-upload'
                | 'data-download'
        ...
        <Modal large hide={!isOpen} onClose={onClose} position="middle">
            <ModalTitle>
                <div className="flex items-center gap-2">
                    {processType
                        ? i18n.t('Failed {{processType}} Details', {
                              processType,
                          })
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <DataTableRow>
                                                                ...
                                                                <DataTableCell>
                                                                    <div
                                                                        className="max-w-[200px] overflow-hidden text-ellipsis whitespace-nowrap font-mono text-xs"
                                                                        title={
                                                                            endpoint
                                                                        }
                                                                    >
