...
export function useFailedQueueDetails(
    configId: string,
    options: {
        limit?: number
        offset?: number
    ...
    const query = useQuery({
        ...
        queryFn: async (): Promise<FailedQueueData> => {
            ...
            if (!response.success) {
                throw new Error(
                    response.message || 'Failed to fetch failed queue data'
                )
            }
    ...
    const clearMutation = useMutation({
        mutationFn: async () => {
            ...
            if (!response.success) {
                throw new Error(
                    response.message || 'Failed to clear failed queue'
                )
            }
            return response
        },
        onSuccess: () => {
            queryClient.invalidateQueries({
                queryKey: ['failed-queue', configId],
            })
            queryClient.invalidateQueries({
    ...
    const retryByTypeMutation = useMutation({
        mutationFn: async (
            ...
            const response = await retryByProcessType(
                engine,
                configId,
                processType
            )
            if (!response.success) {
                throw new Error(
                    response.message ||
                        `Failed to retry ${processType} messages`
                )
            }
            return response
        },
    ...
    const retrySingleMutation = useMutation({
        mutationFn: async (messageId: string) => {
            const response = await retrySingleMessage(
                engine,
                configId,
                messageId
            )
            if (!response.success) {
                throw new Error(
                    response.message || `Failed to retry message ${messageId}`
                )
            }
            return response
        },
