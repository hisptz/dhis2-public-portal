...
import { DataDownloadSummary, DataServiceRunStatus, DataUploadSummary } from "@packages/shared/schemas";
...
export function useDataConfigRunStatus(id: string) {
	...
	const { isLoading, data, error, isError, refetch, isRefetching } = useQuery({
		...
		refetchInterval: (query) => {
			...
			const lastDownloadStatus = last(queryData.download.summaries)?.status;
			const lastUploadStatus = last(queryData.upload.summaries)?.status;
