...
import {
    ...
    useConfiguration,
} from './ConfigurationPage/utils/configurationUtils'
    ...
    const handleImport = useCallback(async () => {
        ...
        try {
            ...
            const jsonFiles = Object.keys(loadedZip.files).filter(
                (filename) =>
                    filename.endsWith('.json') && !loadedZip.files[filename].dir
            )
            ...
            if (documentsInfoFile) {
                const documentsData = JSON.parse(
                    await documentsInfoFile.async('string')
                )
                if (
                    ...
                    for (const doc of documents) {
                        if (doc.attachment && doc.name) {
                            ...
                            if (assetFile) {
                                ...
                                const fileResourceId = await uploadFile(
                                    fileObject,
                                    doc.name,
                                    (message, type) => {
                                        statuses.push({
                                ...
                                if (fileResourceId) {
                                    ...
                                    statuses.push({
                                        ...
                                        message: i18n.t(
                                            'Failed to upload file resource for {{name}}',
                                            { name: doc.name }
                                        ),
                                    })
                                ...
                                statuses.push({
                                    status: 'error',
                                    label: i18n.t('File Resource: {{name}}', {
                                        name: doc.name,
                                    }),
                                    message: i18n.t(
                                        'Asset file {{path}} not found in ZIP',
                                        { path: assetFilePath }
                                    ),
                                })
                                onStatusChange(statuses)
                ...
                if (
                    ...
                    if (
                        ...
                        statuses.push({
                            ...
                            label: `${namespace}`,
                            message: i18n.t(
                                `Successfully imported  ${namespace}`
                            ),
                        })
        ...
        <>
                ...
                <Modal position="middle" onClose={closeModal}>
                    ...
                    <ModalContent>
                        <FileInputField
                            ...
                            placeholder={
                                selectedFile?.name ||
                                i18n.t('No file uploaded yet')
                            }
                            name={i18n.t('importFile')}
                            helpText={i18n.t(
                                'Upload a ZIP file containing Datastore JSON configurations.'
                            )}
            ...
            <TemplateCard
                template={{
                    ...
                    description:
                        'Upload a ZIP file with custom JSON configurations to personalize your portal.',
                    onClick: () => {
                        openModal()
                    },
                }}
