...
import { Box, List, LoadingOverlay, Text } from '@mantine/core'
import {
    IconCircleX,
    IconFile,
    IconFileZip,
...
export function ZIPVisualizer({ path }: ZIPVisualizerProps) {
    ...
    const loadZipContent = useCallback(async () => {
        try {
            setLoading(true)
            setError(null)
            ...
            if (!response.ok) {
                throw new Error(
                    `Failed to load ZIP file: ${response.statusText}`
                )
            }
