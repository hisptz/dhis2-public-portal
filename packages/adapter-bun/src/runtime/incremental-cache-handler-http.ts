import { createFetchPrerenderCacheStore } from './cache-http-client'
import cacheHandler from './cache-handler-http'
import { registerGlobalCacheHandlers } from './cache-handler-registration'
import type { PrerenderTagManifestUpdate } from './isr'
import {
    decodeCacheValue,
...
function resolveRevalidateSeconds(
    ...
    const revalidate = isResponseSetContext(ctx)
...
export default class FetchIncrementalCacheHandler implements NextIncrementalCacheHandler {
    ...
    async revalidateTag(
        ...
        const tags = normalizeTags(
            Array.isArray(tagsInput) ? tagsInput : [tagsInput]
        )
        if (tags.length === 0) return
    ...
    async get(
        ...
        try {
            ...
            console.error('Error decoding cache value:', error)
        }
