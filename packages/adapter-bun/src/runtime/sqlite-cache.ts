...
import { readCacheTagsFromHeaders } from './isr'
...
export class SqlitePrerenderCacheStore implements PrerenderCacheStore {
        ...
        this.#db.transaction(() => {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
                ...
                .run(
                    ...
                    entry.cacheHeaders
                        ? JSON.stringify(entry.cacheHeaders)
                        : null
                )
    ...
    delete(cacheKey: string): void {
        this.#db.transaction(() => {
            this.#db
                .query('DELETE FROM prerender_entries WHERE cache_key = ?')
                .run(cacheKey)
            this.#db
                .query('DELETE FROM revalidate_targets WHERE cache_key = ?')
                .run(cacheKey)
            this.#db
                .query('DELETE FROM revalidate_target_tags WHERE cache_key = ?')
                .run(cacheKey)
    ...
    acquireRevalidateLock(cacheKey: string, ttlSeconds: number): boolean {
        ...
        return this.#db.transaction(() => {
            this.#db
                .query(
                    'DELETE FROM revalidate_locks WHERE cache_key = ? AND expires_at <= ?'
                )
                .run(cacheKey, now)
            ...
            this.#db
                .query(
                    'INSERT INTO revalidate_locks (cache_key, expires_at) VALUES (?, ?)'
                )
...
export class SqliteImageCacheStore implements ImageCacheStore {
    ...
    delete(cacheKey: string): void {
        this.#db
            .query('DELETE FROM image_entries WHERE cache_key = ?')
            .run(cacheKey)
    }
}

export function createSqliteCacheStores(
    options: SqliteCacheOptions & { adapterDir?: string } = {}
): {
    prerenderCacheStore: SqlitePrerenderCacheStore
    imageCacheStore: SqliteImageCacheStore
} {
    const dbPath =
        options.dbPath ?? path.join(options.adapterDir ?? '.', 'cache.db')
