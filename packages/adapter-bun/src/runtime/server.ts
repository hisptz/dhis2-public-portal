...
import { getSharedPrerenderCacheStore } from './cache-store'
import { handleCacheHttpRequest } from './cache-http-server'
...
function resolveCacheRuntimeConfig(manifest: DeploymentManifest): {
    ...
    const handlerMode: CacheHandlerMode =
        cacheConfig?.handlerMode === 'sqlite'
            ? 'sqlite'
            : DEFAULT_CACHE_HANDLER_MODE
    const endpointPath =
        typeof cacheConfig?.endpointPath === 'string' &&
        cacheConfig.endpointPath.length > 0
...
function normalizeCacheControlHeader(
    ...
    const hasNextCacheMarker =
        typeof nextCacheHeaderValue === 'string' &&
        nextCacheHeaderValue.length > 0
    const isDataRequest =
...
async function prepareActionRequestBodyForBun(
    ...
    mutableReq[Symbol.asyncIterator] =
        replayStream[Symbol.asyncIterator].bind(replayStream)
}
...
const projectDir =
    process.env.NEXT_PROJECT_DIR || path.resolve(adapterDir, '..')
...
const appHostname =
    configuredHostname && !isWildcardHostname(configuredHostname)
        ...
        : !isWildcardHostname(listenHostname)
          ...
          : 'localhost'
const protocol =
    process.env.__NEXT_EXPERIMENTAL_HTTPS === '1' ? 'https' : 'http'
...
async function loadRuntimeNextConfig(): Promise<RuntimeConfigRecord> {
    ...
    const distDir =
        typeof serializedConfig.distDir === 'string' &&
        serializedConfig.distDir.length > 0
            ? serializedConfig.distDir
            : typeof manifest.build?.distDir === 'string' &&
                manifest.build.distDir.length > 0
              ? manifest.build.distDir
              : DEFAULT_DIST_DIR

    const handlerModules = getRuntimeHandlerModuleNames(
        cacheRuntime.handlerMode
    )
...
const server = http.createServer(async (req, res) => {
    ...
    if (cacheRuntime.handlerMode === 'http') {
        ...
        if (requestUrl.pathname === cacheRuntime.endpointPath) {
            await handleCacheHttpRequest(
                req,
                res,
                getSharedPrerenderCacheStore(),
                {
