...
import type { PrerenderCacheStore } from './isr'
import {
    ...
    try {
        switch (request.op) {
            ...
            case 'setEntry': {
                await store.set(
                    request.cacheKey,
                    deserializePrerenderCacheEntry(request.entry)
                )
                setJsonResponse(res, 200, {
                    ok: true,
                })
            ...
            case 'findByPrefix': {
                ...
                setJsonResponse(res, 200, {
                    ...
                    entries: entries.map((entry) =>
                        serializePrerenderCacheEntry(entry)
                    ),
                })
                return
            }
