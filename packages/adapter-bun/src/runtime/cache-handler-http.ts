...
import { bytesFromUtf8, decodeBase64ToBytes, utf8FromBytes } from './binary'
import type {
    CacheEntry,
...
function decodeStoredBodyBytes(row: {
    ...
    const encodedBody =
        typeof row.body === 'string' ? row.body : utf8FromBytes(row.body)
...
class FetchCacheHandler implements NextUseCacheHandler {
    async get(
        ...
        let revalidateSec =
            row.revalidateAt !== null
                ? Math.max(
                      0,
                      Math.floor((row.revalidateAt - row.createdAt) / 1000)
                  )
                : 31536000
        const expireSec =
            row.expiresAt !== null
                ? Math.max(
                      0,
                      Math.floor((row.expiresAt - row.createdAt) / 1000)
                  )
                : revalidateSec * 2
    ...
    async set(
        ...
        try {
            ...
            await store.set(cacheKey, {
                ...
                revalidateAt:
                    entry.revalidate > 0
                        ? createdAt + entry.revalidate * 1000
                        : null,
                expiresAt:
    ...
    async getExpiration(tags: string[]): Promise<Timestamp> {
        ...
        for (const tag of tags) {
            ...
            if (!entry) continue
            if (
                entry.expiredAt !== undefined &&
                entry.expiredAt > maxTimestamp
            ) {
