...
import type { BuildCompleteContext, BunStaticAsset } from './types'
...
export async function stageStaticAssets({
    ...
    if (publicStat?.isDirectory()) {
        ...
        for (const publicFilePath of publicFiles) {
            const fileRelativePath = toPosixPath(
                path.relative(publicDir, publicFilePath)
            )
