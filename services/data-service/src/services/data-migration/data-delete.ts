...
import { fetchPagedData, processAttributeComboData } from '@/utils/data'
...
export async function deleteAndQueueData(
    options: DataDeleteOptions
): Promise<void> {
    try {
        const { mainConfigId, dataItemsConfigIds, runtimeConfig } = options
        logger.info(
            `Starting data delete and queue process for config: ${mainConfigId}`
        )
        ...
        logger.info(
            `Data delete jobs successfully queued for config: ${mainConfigId}`
        )
    } catch (error) {
        logger.error(
            `Error during delete and queue process for config ${options.mainConfigId}:`,
            error
        )
        throw error
    }
...
export async function deleteData(jobData: any): Promise<void> {
    try {
        ...
        logger.info(
            `Processing delete job for config: ${mainConfigForClient.id}, period: ${periodId}, config: ${config.id}`
        )
...
async function handleDeletePagination(
    jobData: any,
    client: any
): Promise<boolean> {
    const {
    ...
    logger.info(
        `Delete pagination needed for ${heavyDimension} (${baseDimensions[heavyDimension].length} items > ${pageSize})`
    )
...
async function processDataDeletion(jobData: any, client: any): Promise<void> {
    ...
    if (isEmpty(data.dataValues)) {
        logger.info(
            `No data found to delete in destination for ${config.id}: ${JSON.stringify(dimensions.dx?.slice(0, 5) || 'no dx')}`
        )
        return
    }

    logger.info(
        `Found ${data.dataValues.length} data values in destination to delete for ${JSON.stringify(dimensions.dx?.slice(0, 5) || 'no dx')}`
    )
    ...
    logger.info(
        `${processedData.dataValues.length} data values processed for deletion from destination`
    )

    if (!isEmpty(processedData.dataValues)) {
        await deleteDataValues(processedData.dataValues, mainConfigId)
        logger.info(
            `Successfully deleted ${processedData.dataValues.length} data values from destination`
        )
    }
}

async function processDestinationDataForDeletion(
    data: any,
    dataItems: any[]
): Promise<any> {
    return {
...
async function deleteDataValues(
    dataValues: any[],
    configId: string
    ...
    try {
        ...
        const response = await dhis2Client.post(url, deletionPayload, {
            params,
        })
        ...
        logger.info(
            `Deletion summary for config ${configId}: ${deleted} deleted, ${ignored} ignored, ${updated} updated, ${imported} imported`
        )
        ...
        if (updated > 0 || imported > 0) {
            logger.warn(
                `Unexpected operation counts - updated: ${updated}, imported: ${imported} (should be 0 for DELETE strategy)`
            )
        }
        ...
        logger.error(
            `Error during bulk data deletion for config ${configId}:`,
            error
        )
        throw error
    }
