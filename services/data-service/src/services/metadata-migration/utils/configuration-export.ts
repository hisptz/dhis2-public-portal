 ...
 */
export async function exportAndQueueConfiguration(
    configId: string
): Promise<void> {
    try {
        logger.info(
            `Starting configuration export and queue process for config: ${configId}`
        )
        ...
        logger.error(
            `Error during configuration export and queue process for config ${configId}:`,
            error
        )
        throw error
    }
...
export async function exportConfiguration(
    ...
    try {
        ...
        if (modulesConfig) {
            ...
            const staticNamespaces = await extractStaticModuleNamespaces(
                modulesConfig.keys
            )
        ...
        logger.info(
            `Configuration export completed successfully. Exported ${namespaces.length} namespaces.`
        )
...
async function exportNamespace(
    ...
    try {
        ...
        logger.info(
            `Found ${keys.length} keys in ${namespace}: ${keys.join(', ')}`
        )
        ...
        for (const key of keys) {
            try {
                const valueResponse = await client.get(
 ...
 */
async function extractStaticModuleNamespaces(moduleKeys: {
    [key: string]: any
}): Promise<string[]> {
    ...
    try {
        for (const [moduleId, moduleData] of Object.entries(moduleKeys)) {
            if (moduleData && typeof moduleData === 'object') {
                if (
                    moduleData.type === 'STATIC' &&
                    moduleData.config?.namespace
                ) {
                    const namespace = moduleData.config.namespace
                    logger.info(
                        `Found STATIC module "${moduleId}" with namespace: ${namespace}`
                    )
        ...
        logger.info(
            `Extracted ${staticNamespaces.length} STATIC module namespaces: ${staticNamespaces.join(', ')}`
        )
...
export function validateConfiguration(
    configuration: any
): configuration is ExportedConfiguration {
    try {
        ...
        if (!configuration.exportedAt || !configuration.sourceConfigId) {
            logger.error(
                'Missing required properties: exportedAt or sourceConfigId'
            )
            return false
        }
        ...
        for (const ns of configuration.namespaces) {
            if (!ns.namespace || typeof ns.namespace !== 'string') {
                logger.error(
                    `Invalid namespace structure: ${JSON.stringify(ns)}`
                )
                return false
            }
            ...
            if (!ns.keys || typeof ns.keys !== 'object') {
                logger.error(
                    `Invalid keys structure for namespace ${ns.namespace}`
                )
                return false
            }
        }
