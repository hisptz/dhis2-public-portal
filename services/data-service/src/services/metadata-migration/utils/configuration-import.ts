...
export async function importConfiguration(
    ...
    try {
        ...
        for (const namespaceData of configuration.namespaces) {
            ...
            const result = await importNamespace(
                destinationClient,
                namespaceData
            )
            results.push(result)
        ...
        logger.info(
            `Configuration import completed. ${successfulNamespaces}/${configuration.namespaces.length} namespaces imported successfully in ${duration}ms`
        )
        return summary
    } catch (error) {
...
async function importNamespace(
    ...
    try {
            ...
            try {
                ...
                try {
                    ...
                    if (existingResponse.data) {
                        logger.info(
                            `Key ${namespace}/${key} already exists - updating`
                        )
                    }
                } catch (error) {
                    if (
                        ...
                        logger.info(
                            `Key ${namespace}/${key} doesn't exist - creating`
                        )
                    } else {
                        throw error
                    }
                ...
                logger.warn(
                    `Failed to import key ${namespace}/${key}: ${errorMessage}`
                )
                failedKeys.push(key)
        ...
        logger.info(
            `Namespace ${namespace} import completed: ${successfulKeys}/${totalKeys} keys successful`
        )
...
export async function processConfigurationFromQueue(
    ...
    try {
        logger.info(
            `Processing configuration from queue for config: ${configId}`
        )
        ...
        const configuration =
            queueMessage.configuration as ExportedConfiguration

        // Import the configuration
        const summary = await importConfiguration(configId, configuration)

        logger.info(
            `Configuration processing from queue completed successfully`
        )
        return summary
    } catch (error) {
        logger.error(
            `Error processing configuration from queue for config ${configId}:`,
            error
        )
        throw error
    }
...
export async function validateNamespaceAccess(
    ...
    try {
        ...
        if (error instanceof Error && error.message.includes('404')) {
            logger.info(
                `Namespace ${namespace} does not exist - will be created during import`
            )
            return true
        }
...
export function getImportStatistics(summary: ConfigurationImportSummary): {
    ...
    const totalKeys = summary.results.reduce(
        (sum, result) => sum + result.totalKeys,
        0
    )
    const successfulKeys = summary.results.reduce(
        (sum, result) => sum + result.successfulKeys,
        0
    )
