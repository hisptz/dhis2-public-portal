...
export async function downloadAndQueueMetadata(
    ...
    try {
        ...
        logger.info(
            `Starting metadata download and queue process for config: ${configId}`
        )
        ...
        logger.error(
            `Error during download and queue process for config ${options.configId}:`,
            error
        )
        throw error
    }
...
export async function downloadMetadata(
    ...
    try {
        ...
        if (metadataSource === 'flexiportal-config') {
            ...
            const selectedVisualizationIds =
                options.selectedVisualizations?.map((v) => v.id) || []
            const selectedMapIds = options.selectedMaps?.map((m) => m.id) || []

            let dashboardExtractedIds: {
                visualizationIds: string[]
                mapIds: string[]
            } = { visualizationIds: [], mapIds: [] }
            if (
                options.selectedDashboards &&
                options.selectedDashboards.length > 0
            ) {
                const dashboardIds = options.selectedDashboards.map((d) => d.id)
                dashboardExtractedIds = await getMetadataFromDashboards(
                    dashboardIds,
                    routeId
                )
            }

            const allVisualizationIds = [
                ...new Set([
            ...
            logger.info(
                `Total visualizations to process: ${allVisualizationIds.length}`
            )
        ...
        const mapsConfig =
            maps.length > 0 ? ((await getMapsConfig(maps, routeId)) ?? []) : []

        logger.info('Fetching Visualization Configs...')
        const visualizationConfig =
            visualizations.length > 0
                ? await getVisualizationConfigs(visualizations, routeId)
                : []
        ...
        logger.info(
            `Collected ${dataElementIds.length} unique data element IDs`
        )
        ...
        logger.info(
            `Found ${programIndicatorIds.length} program indicators from indicator sources`
        )
        ...
        const categories = [
            ...indicatorMeta.categories,
            ...dataElementMeta.categories,
        ]
        ...
        const legendSetIds: string[] = [
            ...dataElements
                .map(
                    ({ legendSets }) =>
                        legendSets?.map(({ id }: { id: string }) => id) || []
                )
                .flat(),
            ...indicators
                .map(
                ...
                .flat(),
            ...compact(
                visualizationConfig?.map(({ legend }) => legend?.set?.id)
            ),
            ...compact(
                mapsConfig
                    .map(
        ...
        const legendSets =
            legendSetIds.length > 0
                ? await getLegendSets(uniq(legendSetIds), routeId)
                : []
        ...
        const processedMetadata: ProcessedMetadata = {
            ...
            categories: {
                categories: uniqBy(categories, 'id').filter(
                    (category) =>
                        category.id != sourceDefaults.defaultCategoryId
                ),
                ...
                categoryOptions: uniqBy(categoryOptions, 'id').filter(
                    (option) =>
                        option.id != sourceDefaults.defaultCategoryOptionId
                ),
                categoryOptionCombos: uniqBy(categoryOptionCombos, 'id').filter(
                    (optionCombo: any) =>
                        optionCombo &&
                        optionCombo.name &&
                        !(optionCombo.name as string).includes('default')
                ),
            },
            programIndicatorIds,
        }
        ...
        logger.info(
            `Metadata download completed successfully. Source: ${metadataSource}`
        )
