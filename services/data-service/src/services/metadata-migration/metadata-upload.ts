...
export async function uploadMetadataFromQueue(jobData: any): Promise<void> {
    try {
        ...
        if (!configId) {
            throw new Error(
                `No configId provided in job data. Job data keys: ${Object.keys(jobData || {}).join(', ')}`
            )
        }

        if (type === 'configuration') {
            logger.info(
                `Processing configuration upload for config: ${configId}`
            )
        ...
        if (!metadata) {
            throw new Error(
                `No metadata provided in job data for config: ${configId}. Job data keys: ${Object.keys(jobData || {}).join(', ')}`
            )
        }

        if (!validateMetadata(metadata)) {
            throw new Error(
                `Invalid metadata structure for config: ${configId}`
            )
        }
        ...
        logger.info(
            `Metadata upload job completed successfully for config: ${configId}`
        )
    } catch (error: any) {
...
async function uploadCategoriesMetadata(
    categories: any,
    tempDir: string
): Promise<void> {
    ...
    const totalCategoryOptionCombos =
        categories.categoryOptionCombos?.length || 0

    const totalItems =
        totalCategories +
        totalCategoryOptions +
        totalCategoryCombos +
        totalCategoryOptionCombos
    ...
    if (totalItems <= maxItemsPerUpload) {
        ...
        if (categories.categoryOptionCombos?.length > 0) {
            ...
            await fs.promises.writeFile(
                ...
                JSON.stringify(
                    { categoryOptionCombos: categories.categoryOptionCombos },
                    null,
                    2
                ),
                'utf8'
...
export async function uploadMetadata(
    ...
    try {
        logger.info(
            `Starting metadata upload${configId ? ` for config ${configId}` : ''}...`
        )
        ...
        logger.error(
            `Error during metadata upload${configId ? ` for config ${configId}` : ''}:`,
            error
        )
        throw error
    }
...
export function validateMetadata(metadata: any): metadata is ProcessedMetadata {
    try {
        ...
        if (
            !metadata.dataItems.indicators ||
            !metadata.dataItems.dataElements
        ) {
