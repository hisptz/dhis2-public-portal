	...
	try {
		...
		const task =
			runType === "metadata"
				...
				: runType === "data" ? await dbClient.dataUpload.findUnique({
					where: { uid: taskId },
					select: { filename: true }
				}) : undefined;
		...
		logger.error(
			`Failed to get file for task ${req.params.taskId}:`,
			error
		);
...
GET.apiDoc = {
	...
	description: "Allows downloading the file associated with a failed upload task for debugging purposes.",
	tags: ["Data Service"],
	parameters: [
		{
			name: "configId",
