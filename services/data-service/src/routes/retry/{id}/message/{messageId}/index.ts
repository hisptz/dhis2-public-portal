...
async function findAndRetrySpecificMessage(
    configId: string,
    ...
    try {
        ...
        if (retryResult.success) {
            ...
            channel.sendToQueue(
                failedQueueName,
                messageBuffer,
                actualTargetMessage.properties
            )
...
async function getFailedMessageById(
    configId: string,
    targetMessageId: string
): Promise<any | null> {
    const queueNames = getQueueNames(configId)
...
async function retryMessage(
    ...
    try {
        ...
        logger.info(
            `Successfully retried specific message to queue: ${sourceQueue}`
        )
        return { success: true }
    } catch (error: any) {
...
export const POST: Operation = async (
    ...
    try {
        ...
        logger.info(
            `Individual message retry request for config: ${configId}, message: ${messageId}`
        )
        ...
        res.status(500).json({
            ...
            message:
                error.message ||
                'An unexpected error occurred during message retry',
            configId: req.params.id,
            messageId: req.params.messageId,
        })
    }
...
export const GET: Operation = async (
    ...
    try {
        ...
        logger.info(
            `Getting failed message details for config: ${configId}, message: ${messageId}`
        )
