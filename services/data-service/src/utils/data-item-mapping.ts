...
import { getDefaultCategoryValues } from './default-categories'
...
export async function generateAndSaveDataItemMappings(
    ...
    try {
        ...
        const dataElementIds = metadata.dataItems.dataElements.map(
            (de: any) => de.id
        )
        ...
        if (dataElementIds.length === 0 && programIndicatorIds.length === 0) {
            logger.info(
                `No data elements or program indicators found, skipping mapping generation`
            )
            return
        }
        logger.info(
            `Generating mappings for ${dataElementIds.length} data elements and ${programIndicatorIds.length} program indicators`
        )
        ...
        logger.info(
            `Successfully generated and saved ${mappings.length} data item mappings`
        )
    } catch (error) {
...
export async function generateDataItemMappings(
    ...
    try {
        ...
        if (dataElementIds.length > 0) {
            try {
                const dataElementMappings = await generateDataElementMappings(
                    dataElementIds,
                    routeId
                )
                mappings.push(...dataElementMappings)
                logger.info(
                    `Successfully generated ${dataElementMappings.length} data element mappings`
                )
            } catch (error) {
        ...
        if (programIndicatorIds.length > 0) {
            try {
                const programIndicatorMappings =
                    generateProgramIndicatorMappings(programIndicatorIds)
                mappings.push(...programIndicatorMappings)
                logger.info(
                    `Successfully generated ${programIndicatorMappings.length} program indicator mappings`
                )
            } catch (error) {
                logger.error(
                    `Failed to generate program indicator mappings:`,
                    error
                )
            }
        }
        ...
        logger.info(`Data item mapping generation completed`, {
            ...
            dataElementMappings: mappings.filter(
                (m) => !m.sourceId.includes('.')
            ).length,
            categoryComboMappings: mappings.filter((m) =>
                m.sourceId.includes('.')
            ).length,
        })
