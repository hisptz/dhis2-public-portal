...
let sourceDefaults: Awaited<
    ReturnType<typeof getDefaultCategoryValues>
> | null = null
let destinationDefaults: Awaited<
    ReturnType<typeof getDestinationDefaultCategoryValues>
> | null = null
...
export async function getCategoryCombos(
    categoryOptionComboIds: string[],
    routeId?: string
) {
    const client = routeId ? await createSourceClient(routeId) : dhis2Client
    const categories = []
    ...
    logger.info(
        `Successfully fetched ${fetchedCategoryOptionCombos.length} category option combos`
    )
    ...
    logger.info(
        `Extracted ${categoryComboIds.length} unique category combo IDs`,
        {
    ...
    logger.info(
        `Successfully fetched ${fetchedCategoryCombos.length} category combos`
    )
    ...
    const categoryOptionIds = uniq(
        fetchedCategories
            .map(({ categoryOptions }: any) =>
                categoryOptions.map(({ id }: any) => id)
            )
            .flat()
    )
    ...
    categories.push(
        ...fetchedCategories.map((category: any) => ({
            ...category,
            categoryOptions: category.categoryOptions.map(({ id }: any) => ({
                id,
            })),
        }))
    )
...
export async function getDataElementsForProgramIndicators(
    ...
    const dataElements = programIndicators.map((programIndicator: any) => {
        return {
    ...
    logger.info(
        `Successfully converted ${dataElements.length} program indicators to data elements`
    )
    return {
        dataElements,
    }
...
export async function getCategoryCombosFromDataElements(
    ...
    logger.info(
        `Source default category combo id: ${sourceDefaults.defaultCategoryComboId}`
    )
    const client = routeId ? await createSourceClient(routeId) : dhis2Client
    ...
    if (categoryComboIds.length === 0) {
        logger.info(
            'No non-default category combos found, returning empty results'
        )
        return {
            categories: [],
            categoryOptions: [],
...
export async function getIndicatorsSources(
    ...
    const categoryMeta = await getCategoryCombos(
        uniq(categoryOptionComboIds),
        routeId
    )
    ...
    logger.info(
        `Successfully converted ${dataElementsFromProgramIndicators.length} program indicators to data elements`
    )

    const indicatorTypesResult = await getIndicatorTypes(
        uniq(indicatorTypes),
        routeId
    )
    logger.info(
        `Successfully fetched ${indicatorTypesResult.length} indicator types`
    )
    return {
